...
rampRoutes.route('/app/ramp/add/multiple').post(async (req, res, next) => {
    ...
    try {
        ...
        const insertPromises = ramps.map(ramp => {
            const queryText = `
                INSERT INTO Ramp (latitude, longitude, building)
                VALUES ($1, $2, $3)
                RETURNING *;
            `;
            return client.query(queryText, [ramp.latitude, ramp.longitude, ramp.building]);
        });

        const results = await Promise.all(insertPromises);
        await client.query('COMMIT');

        const insertedRamps = results.map(result => result.rows[0]);
        res.status(200).json({
